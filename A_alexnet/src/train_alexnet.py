...
if __name__ == "__main__":
    ...
    # config
    data_dir = os.path.join(BASE_DIR, "..", "data", "dogs-vs-cats-redux-kernels-edition", "train")
    ...
    LR = 0.001          # 可自行修改
    log_interval = 1    # 多少iter打印一次情况 可自行修改
    val_interval = 1    # 多少个iter后运行一次验证集 可自行修改
    classes = 2
    start_epoch = -1    # 一开始+1, 因此是从零开始
    ...
    train_transform = transforms.Compose([
        transforms.Resize((256)),      # (256, 256) 区别 如果长宽比不是1：1, 如果只有一个数字, 会按照短边进行缩放, 原图长宽比例会仍然保存
        transforms.CenterCrop(256),    # 为了使得其fixed-size 256x256输入，我们再进行一步裁剪, 中心截取256x256区域
        transforms.RandomCrop(224),    # 1024种可能 (256 - 224)^2
        transforms.RandomHorizontalFlip(p=0.5), # 1024 x 2 = 2048
        transforms.ToTensor(),
    ...
    valid_transform = transforms.Compose([
        ...
        # Tencrop这里会生成一个10张图片的list, 要将其变成10张separate的图片进行输入才行 BCHW形式
        transforms.TenCrop(224, vertical_flip=False),
        # 用这里的正则表达式 再使用stack来组装起来变成维度 (B, 10, C, H, W) 到时候还需要再处理
    ...
    # 变更最后一层为二分类，我们使用一个linear层覆盖以前的最后一层
    ...
    if flag:
        ...
        optimizer = optim.SGD([
            {'params': base_params, 'lr': LR * 0.1},  # 0 设置为0时只更新分类器,前面参数不更新
        ...
        if (epoch+1) % val_interval == 0:
            ...
            with torch.no_grad():
                for j, data in enumerate(valid_loader):
                    ...
                    # Tencrop之后得到的是一个5D张量 要重新变成BCHW的形式 valid中batch_size为4
                    bs, ncrops, c, h, w = inputs.size()     # [4, 10, 3, 224, 224]
                    outputs = alexnet_model(inputs.view(-1, c, h, w)) # [40, 3, 224, 224]
                    # [4, 10, 2] 的shape, 对一张图片生成的10个crop取mean, (mean(1))就是对第二个数求mean
